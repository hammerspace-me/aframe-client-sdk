...
const PROVIDER_LOADERS = {
    "ready-player-me": function (avatar, avatarEntity) {
    ...
    "crypto-avatars": function (avatar, avatarEntity) {
        avatarEntity.setAttribute("hammerspace-restrict-rotation", {
            offset: 0,
        });
    },
    meebits: function (avatar, avatarEntity) {
        avatarEntity.setAttribute("hammerspace-restrict-rotation", {
            offset: 0,
        });
    },
...
AFRAME.registerComponent("hammerspace-avatar", {
    ...
    init: async function () {
        ...
        const avatarId = this.data.itemId;
        ...
        try {
            ...
            const hammerspaceAvatar = await getHammerspaceAvatar(
                avatarId,
                this.data.hammerspaceUrl
            );
            ...
            const providerLoader = PROVIDER_LOADERS[hammerspaceAvatar.source];
            if (providerLoader) {
            ...
            this.avatarEntity.setAttribute(
                "gltf-model",
                this.data.ipfsGateway + "/" + hammerspaceAvatar.content
            );
        } catch (error) {
            console.error(error);
            this.setDefaultAvatar();
...
AFRAME.registerComponent("hammerspace-avatar-selector", {
    ...
    init: async function () {
        ...
        try {
            const items = await this.getItems(this.data.accessToken);
            ...
            this.el.emit("hammerspace:error", error);
        }
    },
    createAvatar: function (cid, position, rotation, id, source) {
        ...
        mainEl.addEventListener("click", (event) => {
            this.selected = cid;
            this.el.emit("hammerspace:avatar-selected", id);
    ...
    removeAvatar(cid) {
        for (avatarEl in this.avatarEls) {
            if (avatarEl.cid === cid) {
                ...
                this.avatarEls = this.avatarEls.filter(
                    (elem) => elem.cid !== cid
                );
                return avatarEl;
            }
    ...
    getItems: async function (accessToken) {
        const response = await fetch(
            this.data.hammerspaceUrl + "/space/owner",
            {
                headers: {
